...
const startIdx = content.indexOf('projects: [');
const endIdx = content.indexOf('],', startIdx) + 2;

if (startIdx !== -1 && endIdx !== -1) {
  content = content.substring(0, startIdx) + newProjects + content.substring(endIdx);
  fs.writeFileSync(dataJsPath, content, 'utf8');
  console.log('Successfully updated projects array.');
} else {
  console.error('Could not find projects array in data.js');
}
